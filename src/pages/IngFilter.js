...
function IngFilter() {
  ...
  const fetchIngsFilter = async () => {
    setLoading(true)
    try {
      const res = await fetch(`${url}${ingredient}`)
  ...
  if(filter.length < 1){
    return (
      <h2 className='section-title'>there are no drinks under {ingredient}</h2>
    )
  }
  return (
    <section className='section'>
      <h2 className='section-title'>Drinks Made with {ingredient}</h2>
